...
const ExpenseManager = ({ property, properties, onSaveData, loans, transactions, settings }) => {
  ...
  const handleSave = async () => {
    ...
    let updatedExpenses;
    let updatedTransactions = [...(Array.isArray(transactions) ? transactions : [])];
    ...
    if (editingExpense) {
      ...
      // FIXED: Update corresponding transaction using inline builder
      const existingTransactionIndex = updatedTransactions.findIndex(t => 
        t.expenseId === editingExpense.id
      );
      
      if (existingTransactionIndex >= 0) {
        // Update existing transaction
        updatedTransactions[existingTransactionIndex] = __makeTx({
          ...
          type: "expense",
          meta: {
            id: updatedTransactions[existingTransactionIndex].id, // Keep original transaction ID
        ...
        updatedTransactions.push(__makeTx({
          propertyId: property.id,
          amount: expenseData.amount,
          date: expenseData.date,
          category: expenseData.category,
      ...
      // FIXED: Create corresponding transaction using inline builder
      updatedTransactions.push(__makeTx({
        propertyId: property.id,
        amount: expenseData.amount,
        date: expenseData.date,
  ...
  const handleDelete = async (expenseId) => {
    ...
    // FIXED: Remove corresponding transaction
    const updatedTransactions = (transactions || []).filter(t => t.expenseId !== expenseId);
